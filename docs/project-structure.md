...
├── identification/           ← catalog files + instructions for Claude Desktop import workflow
│   ├── claude-desktop-project-instructions.md
│   ├── he-man-catalog.json
│   ├── star-wars-catalog.json
│   └── transformers-catalog.json
...
### `docs/identification/`
Files supporting the one-time bulk import of physical collection items via Claude Desktop photo identification (COL-127).

- **`claude-desktop-project-instructions.md`** — paste into Claude Desktop project instructions; defines the identification output format, uncertainty rules, JSON schema, and per-collection defaults
- **`he-man-catalog.json`** / **`star-wars-catalog.json`** / **`transformers-catalog.json`** — lean identification-focused catalogs (all irrelevant fields stripped) generated by `scripts/export-identification-catalogs.ts`; load as project knowledge in Claude Desktop
