...
- **MCP staging server:** `https://stage-mcp.houseoffunk.net`, port 3002, pm2 process `mcp-server-stage` in `~/Sites/my-collections-stage/packages/mcp`. Production (when deployed): `https://mcp.houseoffunk.net`, port 3003, pm2 `mcp-server`. Bearer token in `packages/mcp/.env` on the Mini (not committed).
...
- **MCP token reuse detection — nuclear response:** The crash loop can cause a secondary failure. If a restart gets through the rate limit, it rotates the refresh token (invalidates old, writes new to `.env`), then crashes before the new token is used cleanly. The next restart presents the already-used token — the API interprets this as token theft and **revokes ALL refresh tokens for that user + client**. Symptom: 401 "Refresh token already used — possible token theft" in pm2 logs. Recovery: `pm2 stop`, re-run `npm run bootstrap --workspace=packages/mcp` with user credentials, then `pm2 start`. The 429 backoff retry does NOT help with 401 — those crash immediately.
