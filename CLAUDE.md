...
- Collections API complete (COL-10): full CRUD for Star Wars, Transformers, He-Man; catalog seeded (199 SW / 443 TF / 127 HM)
- Production deployed on Mac Mini: API at `https://api.houseoffunk.net`, web at `https://collections.houseoffunk.net`; staging at `stage-api` / `stage.houseoffunk.net`
...
- **MCP server** (`packages/mcp`, `@my-collections/mcp`): Plain TypeScript Node.js server (not NestJS) wrapping the Collections REST API. Uses Express + `@modelcontextprotocol/sdk` with `StreamableHTTPServerTransport`. Hosted at `mcp.houseoffunk.net:3001` (pm2 process: `mcp-server`). Auth is two-layer: static bearer token (Claude → MCP) + stored refresh token (MCP → API). First-time setup: run `npm run bootstrap` in `packages/mcp` — prompts for credentials via readline (no browser; the API has no HTML login page) and writes `MCP_REFRESH_TOKEN` to `packages/mcp/.env`. Register in claude.ai Settings → Integrations → MCP Servers. See `docs/confluence/mcp-server.md` for the full tool catalog and compound use case examples.
- **MCP SDK Zod import:** Tool `inputSchema` in `@modelcontextprotocol/sdk` v1.29.0 is typed against `import type * as z3 from 'zod/v3'` internally. Always import as `import { z } from 'zod/v3'` in tool files — using the root `'zod'` import causes a TypeScript structural type mismatch at compile time. The installed `zod` v3.25.x ships both `/v3` and `/v4` subpaths.
- **MCP SDK inputSchema shape:** Pass a **raw Zod object shape** to `inputSchema`, not a `z.object()` call. Correct: `inputSchema: { limit: z.number().optional() }`. Wrong: `inputSchema: z.object({ limit: z.number().optional() })`. The SDK wraps the shape in `z.object()` internally.
- **MCP StreamableHTTPServerTransport import path:** `import { StreamableHTTPServerTransport } from '@modelcontextprotocol/sdk/server/streamableHttp.js'`; `import { McpServer } from '@modelcontextprotocol/sdk/server/mcp.js'`. Note the `.js` extension in the import path (required for the SDK's ESM exports even in a CJS consumer).
- **MCP staging server:** `https://stage-mcp.houseoffunk.net`, port 3002, pm2 process `mcp-server-stage` in `~/Sites/my-collections-stage/packages/mcp`. Production: `https://mcp.houseoffunk.net`, port 3003, pm2 `mcp-server` in `~/Sites/my-collections/packages/mcp`. Both run in **fork mode** (`exec_mode: 'fork'` in ecosystem.config.js). Bearer token in `packages/mcp/.env` on the Mini (not committed).
- **MCP server pm2 crash loop:** The token manager calls `/auth/token` on every startup. If pm2 auto-restarts a crashing MCP process, it hammers the rate-limited token endpoint (10 req/min) until every startup gets 429 — a self-perpetuating cycle. Always `pm2 stop <mcp-process>` before rebuilding dist/, then `pm2 start` once. The token manager has exponential backoff (2s→4s→8s→16s→32s→60s, 20 attempts max) to self-recover from brief 429s without crashing.
- **MCP token reuse detection — root cause and recovery:** pm2 cluster mode briefly runs two workers simultaneously during a restart; both read the same refresh token from `.env` and race to call `/auth/token`. The second call triggers token theft detection — the API **revokes ALL refresh tokens for that user + client**. This is why both MCP processes use `exec_mode: 'fork'`. If you ever see 401 "Refresh token already used — possible token theft" in pm2 logs: `pm2 stop <mcp-process>`, re-run `npm run bootstrap --workspace=packages/mcp` interactively (needs `-t` flag via SSH), then `pm2 start`. The 429 backoff retry does NOT help with 401 — those crash immediately. Never pull `develop` directly onto the prod server (`~/Sites/my-collections`) — always go through a develop → main PR so CI handles the deploy.
- **MCP `get_collection_stats` field name:** API returns `estimatedTotalValue` (nullable), not `estimatedValue`. Use null-safe formatting — the field is `null` when no items have values set.
- **MCP stateless transport:** Use `new StreamableHTTPServerTransport({ sessionIdGenerator: undefined })` for stateless mode. Wire with `app.all('/mcp', async (req, res) => transport.handleRequest(req, res, req.body))` — both GET and POST go to the same route; the SDK dispatches internally. Do NOT create separate GET/POST handlers.
...
- **expo must be hoisted in monorepo for EAS:** `expo-router`'s config plugin lives at root `node_modules` and requires `expo/config-plugins`. If `expo` is only at `packages/mobile/node_modules`, the plugin fails with `Cannot find module 'expo/config-plugins'`. Fix: add `expo: ~55.0.25` to root `devDependencies` and run `npm install`. Then commit the updated `package-lock.json` — EAS uses `npm ci` which requires lock file sync.
- **`unmountOnBlur` removed in React Navigation v7:** Setting `unmountOnBlur` on a Tab screen is silently ignored in React Navigation v7 (expo-router 55+). The replacement is `popToTopOnBlur: true`, which pops the nested Stack to its root screen whenever the tab blurs — the correct fix for stale cross-tab navigation history accumulating in a Stack nested under Tabs.
...
- **Versioning:** Both `packages/api/package.json` and `packages/web/package.json` carry a semver version (currently `1.0.0`). Bump both on each production deploy: patch (1.0.x) for bug fixes, minor (1.x.0) for new features, major (x.0.0) for breaking changes requiring coordinated frontend + API update. `GET /health` exposes the API version at runtime; `__APP_VERSION__` (injected by Vite via `define`) bakes the SPA version into the build. The dashboard footer displays both.
