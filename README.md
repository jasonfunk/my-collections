...
- Phases 1–5 complete: CI/CD, local DB + migrations, OAuth2 auth, Collections API, React SPA with full CRUD (browse, detail, add, edit, wishlist, search, PDF export)
- Phase 6 (mobile) in progress: login, dashboard, browse, item detail, add/edit/delete, wishlist, global search, and catalog images on browse/detail complete (COL-47 through COL-52, COL-107, COL-108, COL-111); EAS Build configured for physical device distribution (preview APK + production AAB profiles); barcode scanning and push notifications remain
