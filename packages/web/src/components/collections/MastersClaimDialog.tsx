...
function makeDefaults(existing?: UserMastersItem): ClaimFormState {
  if (existing) {
    return {
      ...
      isCarded: existing.isCarded,
      hasBackCard: existing.hasBackCard,
...
interface MastersClaimDialogProps {
  catalogItem: MastersCatalogItem;
  existing?: UserMastersItem;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function MastersClaimDialog({
  catalogItem,
  existing,
  open,
  onOpenChange,
  onSuccess,
}: MastersClaimDialogProps) {
  ...
  function buildDto() {
    return {
      ...
      isCarded: form.isCarded,
      hasBackCard: form.hasBackCard,
  ...
  const mutation = useMutation({
    mutationFn: (dto: unknown) =>
      existing
        ? apiClient.patch<UserMastersItem>(`/collections/he-man/items/${existing.id}`, dto)
        : apiClient.post<UserMastersItem>('/collections/he-man/items', dto),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['hm-user-items'] });
      void queryClient.invalidateQueries({ queryKey: ['hm-catalog-item', catalogItem.id] });
      onSuccess();
      onOpenChange(false);
    },
  ...
  const title = existing
    ? `Edit — ${catalogItem.name}`
    : `Claim — ${catalogItem.name}`;
          ...
          <div className="space-y-5 py-2">
            ...
            {form.isOwned && (
              <>
                ...
                {/* Flags — He-Man uses hasBackCard instead of isBoxed */}
                <div className="flex flex-wrap gap-6">
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="claim-isComplete"
                      checked={form.isComplete}
                      onCheckedChange={(v) => patch({ isComplete: !!v })}
                    />
                    <Label htmlFor="claim-isComplete">Complete</Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="claim-isCarded"
                      checked={form.isCarded}
                      onCheckedChange={(v) => patch({ isCarded: !!v })}
                    />
                    <Label htmlFor="claim-isCarded">Carded</Label>
                  </div>
                  <div className="flex items-center gap-2">
                    <Checkbox
                      id="claim-hasBackCard"
                      checked={form.hasBackCard}
                      onCheckedChange={(v) => patch({ hasBackCard: !!v })}
                    />
                    <Label htmlFor="claim-hasBackCard">Back card intact</Label>
                  </div>
                </div>

                {/* Accessories from catalog */}
