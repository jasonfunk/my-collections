...
import type { StarWarsCatalogItem, UserStarWarsItem } from '@my-collections/shared';
import {
  AcquisitionSource,
  ConditionGrade,
  PackagingCondition,
...
function makeDefaults(existing?: UserStarWarsItem): ClaimFormState {
  if (existing) {
    return {
...
interface StarWarsClaimDialogProps {
  catalogItem: StarWarsCatalogItem;
  existing?: UserStarWarsItem;
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSuccess: () => void;
}

export function StarWarsClaimDialog({
  catalogItem,
  existing,
  open,
  onOpenChange,
  onSuccess,
}: StarWarsClaimDialogProps) {
  ...
  const mutation = useMutation({
    mutationFn: (dto: unknown) =>
      existing
        ? apiClient.patch<UserStarWarsItem>(`/collections/star-wars/items/${existing.id}`, dto)
        : apiClient.post<UserStarWarsItem>('/collections/star-wars/items', dto),
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ['sw-user-items'] });
      void queryClient.invalidateQueries({ queryKey: ['sw-catalog-item', catalogItem.id] });
      onSuccess();
      onOpenChange(false);
    },
        ...
        <form onSubmit={handleSubmit} noValidate>
          <div className="space-y-5 py-2">
                ...
                <div className="flex flex-wrap gap-6">
                  ...
                  <div className="flex items-center gap-2">
                    <Checkbox
                      ...
                      onCheckedChange={(v) => patch({ isBoxed: !!v })}
                    />
                    <Label htmlFor="claim-isBoxed">Boxed</Label>
                  </div>
                </div>
            ...
            <div className="space-y-3">
              ...
              {/* Est. value — visible in both owned and wishlist mode */}
              <div className="space-y-1">
              ...
              {/* Source, date, price paid — owned only */}
              {form.isOwned && (
                <>
                  <div className="space-y-1">
                    <Label>Source</Label>
