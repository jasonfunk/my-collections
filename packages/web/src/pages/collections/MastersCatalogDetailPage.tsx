...
import { MastersClaimDialog } from '@/components/collections/MastersClaimDialog.js';
...
export function MastersCatalogDetailPage() {
  ...
  const { data: catalogItem, isPending: catalogPending, isError: catalogError } = useQuery({
    queryKey: ['hm-catalog-item', id],
    queryFn: () => apiClient.get<MastersCatalogItem>(`/collections/he-man/catalog/${id}`),
    enabled: !!id,
  });
  ...
  const deleteMutation = useMutation({
    mutationFn: () => apiClient.delete(`/collections/he-man/items/${userItem!.id}`),
    onSuccess: () => {
    ...
    <div className="min-h-screen bg-muted/40">
      <header className="border-b bg-background px-6 py-4">
        <div className="mx-auto flex max-w-5xl items-center justify-between">
          <Button variant="ghost" size="sm" onClick={() => navigate('/collections/he-man')}>
                ...
                <div className="space-y-3">
                  ...
                  {userItem.isOwned && (
                    <div className="space-y-2">
                      ...
                      <BoolRow label="Back card intact" value={userItem.hasBackCard} />
            ...
            <div>
              ...
              <p className="mt-1 text-sm text-muted-foreground">
                {catalogItem.characterType
                  ...
                  : 'Item'}
                {catalogItem.line && ` · ${MASTERS_LINE_LABELS[catalogItem.line] ?? catalogItem.line}`}
              </p>
            </div>
            ...
            <section>
              ...
              <div className="space-y-2">
                {catalogItem.releaseYear && (
                  <DetailRow label="Release year" value={String(catalogItem.releaseYear)} />
                )}
                {catalogItem.miniComic && (
                  <DetailRow label="Mini-comic" value={catalogItem.miniComic} />
                )}
                {catalogItem.hasArmorOrFeature && (
                  <DetailRow label="Action feature" value={catalogItem.featureDescription ?? '✓ Yes'} />
                )}
