 ...
 *
 * Run from repo root:
 *   npm run seed:he-man              # insert new records only
 *   npm run seed:he-man -- --update  # also update all nullable fields on existing rows
 */
...
async function seed() {
  ...
  if (updateMode) {
    ...
    for (const record of records) {
      const entityData = record as Partial<MastersCatalogEntity>;
